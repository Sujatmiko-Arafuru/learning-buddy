  ...
  const resolveNextRoute = async (email: string) => {
    try {
      ...
      const hasSkillAssessment =
        Boolean(user?.skill_assessment && Object.keys(user.skill_assessment).length > 0);
      ...
      if (hasProgress || hasSkillAssessment) return '/dashboard';
      if (hasPersonalization) return '/dashboard';
      return '/personalize';
    } catch (error) {
    ...
    try {
      ...
      if (!err?.response) {
        if (err?.message?.includes('Network Error') || err?.message?.includes('Cannot connect')) {
          errorMessage =
            'Network Error: Backend server tidak dapat dijangkau. Pastikan backend berjalan di http://localhost:5000';
  ...
  return (
    <Container>
      <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '90vh' }}>
        <div
          className="shadow-lg bg-white rounded overflow-hidden d-flex flex-column flex-md-row"
        ...
        >
          ...
          {/* ---------- KANAN: FORM LOGIN ---------- */}
          <div className="card border-0 rounded-0" style={{ width: '50%' }}>
