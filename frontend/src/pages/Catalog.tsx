...
import {
  ...
  Badge,
  Button,
} from "react-bootstrap"; // Tambah Button
...
const Catalog: React.FC = () => {
  ...
  const [selectedMapInterests, setSelectedMapInterests] = useState<
    MapInterest[]
  >([]);
    ...
    try {
      ...
      if (userEmail) {
        try {
          ...
          const selectedIds =
            user?.preferences?.selected_learning_path_ids || [];
          ...
          if (selectedIds.length > 0) {
            ...
            const courseData = await learningPathApi.getCourses(
              undefined,
              selectedIds
            );
      ...
      setLearningPaths([
        ...
        {
          learning_path_id: 3,
          learning_path_name: "Back-End Developer JavaScript",
        },
      ]);
      setCourses([
        {
        ...
        <Card className="mb-4 border-primary">
          ...
          <Card.Body>
            ...
            <p className="text-muted small mt-2 mb-0">
              Menampilkan course dari {userSelectedLpIds.length} Learning Path
              yang sesuai dengan Map Interest yang dipilih
            </p>
          </Card.Body>
        </Card>
      )}
              ...
              <Card className="h-100">
                <Card.Body>
                  ...
                  <p className="text-muted small mb-2">
                    {learningPaths.find(
                      (lp) => lp.learning_path_id === course.learning_path_id
                    )?.learning_path_name ||
                  ...
                  <Button
                    ...
                    onClick={() =>
                      navigate(
                        `/course/${encodeURIComponent(course.course_name)}`,
                        { state: { fromCatalog: true } } // <-- Tambah state ini
                      )
                    }
                  >
                    Lihat Detail
