...
def get_exam_questions():
    ...
    try:
        ...
        exam_coll = _get_collection('Soal_Ujian')
        if exam_coll is None:
            return jsonify({'success': False, 'error': 'Soal_Ujian collection not found'}), 404
       
        # Ambil semua soal untuk course ini
        all_questions = list(exam_coll.find(
        ...
        if not all_questions:
            return jsonify({
                'success': False,
                'error': f'Tidak ada soal ujian ditemukan untuk course: {decoded_course_name}'
            }), 404
    ...
    try:
        ...
        for i, answer_data in enumerate(answers):
            ...
            if question:
                ...
                print(f"  ⚠️ Question not found in questions data")
                detailed_results.append({
